<template>
  <div class="dashboard">
    ...
    <form @submit.prevent="handleSubmit">
      ...
      <div v-if="form.imageGallery.length" class="gallery-strip" ref="galleryRef">
...
// Hero image file select - save file for upload later
...
// Save selected gallery files for upload later
const handleGalleryUpload = (e) => {
  const files = Array.from(e.target.files);
  if (files.length === 0) return;
  pendingGalleryFiles.value.push(...files);
};

const removeGallery = (idx) => {
  form.value.imageGallery.splice(idx, 1);
  // Also remove from pendingGalleryFiles if relevant
  if (idx < pendingGalleryFiles.value.length) {
    pendingGalleryFiles.value.splice(idx, 1);
  }
};

// Change one gallery image file, update pending files accordingly
const changeGalleryImage = (e, idx) => {
  const file = e.target.files[0];
  if (!file) return;
  // Replace file in pendingGalleryFiles at idx if exists, else upload immediately
  if (idx < pendingGalleryFiles.value.length) {
    pendingGalleryFiles.value.splice(idx, 1, file);
  } else {
    // fallback: upload immediately and replace in form.imageGallery
    uploadSingle(file).then(url => {
      form.value.imageGallery.splice(idx, 1, url);
    });
  }
};

// Upload a single file to Cloudinary and get URL
...
// Upload all pending files and clear pending arrays
...
const handleSubmit = async () => {
  ...
  const data = {
    ...
    bedrooms: +form.value.bedrooms,    description: form.value.description,    mainImage: form.value.mainImage,    imageGallery: form.value.imageGallery,    createdAt: new Date(),  };  if (form.value.id) {    await updateDoc(doc(db, 'properties', form.value.id), data);  } else {    await addDoc(collection(db, 'properties'), data);  }  await fetchProperties();  resetForm();};
...
onMounted(() => {
  ...
  Sortable.create(el, {
    animation: 150,
    onEnd: (evt) => {
      const movedItem = form.value.imageGallery.splice(evt.oldIndex, 1)[0];
      form.value.imageGallery.splice(evt.newIndex, 0, movedItem);
    }
  });
});
</script>
